...
class LabelsControllerTest < ActionController::TestCase
  ...
  test "should create label" do
    assert_difference('Label.count') do
      post :create, label: { description: @label.description, fullname: @label.fullname, name: @label.name }
    end
  ...
  test "should update label" do
    patch :update, id: @label, label: { description: @label.description, fullname: @label.fullname, name: @label.name }
    assert_redirected_to label_path(assigns(:label))
  end
